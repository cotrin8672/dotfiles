...
local function render()
	...
	elseif not state.supported then
		...
		local width = is_visible() and vim.api.nvim_win_get_width(state.winid) or dimensions().width
...
local function schedule_request(kind)
	...
	vim.defer_fn(function()
		...
		state.pending_request = nil
		if not is_visible() or not state.connected or not state.supported then
			state.dirty = true
			return
		end
...
function M.handle_server_message(result, client_id)
	...
	elseif result.type == "Columns" then
		...
		if is_visible() and state.rows ~= nil then
			render()
		end
