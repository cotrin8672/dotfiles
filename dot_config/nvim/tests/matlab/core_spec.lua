describe("MATLAB execution client", function()
	...
	it("creates independent sessions and routes work to the selected client", function()
		assert.is_true(core.enqueue_eval("session one", { bufnr = 1 }))
		core.handle_mvm_state_change({ state = "connected", release = "R2024a" }, 41)
		core.handle_eval_response({ requestId = sent[1].params.requestId }, 41)

		assert.is_true(core.new_session(1))
		assert.are.equal(2, core._snapshot().session_count)
		assert.are.equal(2, core._snapshot().session_index)
		assert.are.equal(42, core._snapshot().client_id)
		core.handle_mvm_state_change({ state = "connected", release = "R2024a" }, 42)
		assert.is_true(core.enqueue_eval("session two", { bufnr = 1 }))
		assert.are.equal(42, sent[#sent].client_id)

		assert.is_true(core.previous_session())
		assert.are.equal(1, core._snapshot().session_index)
		assert.are.equal(41, core._snapshot().client_id)
		assert.is_true(core.enqueue_eval("back on one", { bufnr = 1 }))
		assert.are.equal(41, sent[#sent].client_id)

		assert.is_true(core.next_session())
		assert.are.equal(42, core._snapshot().client_id)
	end)

	it("removes a stopped session and selects the following session", function()
		assert.is_true(core.ensure_client(1))
		core.handle_mvm_state_change({ state = "connected", release = "R2024a" }, 41)
		assert.is_true(core.new_session(1))
		core.handle_mvm_state_change({ state = "connected", release = "R2024a" }, 42)
